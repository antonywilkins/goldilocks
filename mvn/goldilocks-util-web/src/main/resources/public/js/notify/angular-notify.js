angular.module('cgNotify', []).factory('notify',['$timeout','$http','$compile','$templateCache','$rootScope',
    function($timeout,$http,$compile,$templateCache,$rootScope){
        ...
        var notify = function(args){
            ...
            args.duration = typeof args.duration == "number" ? args.duration : defaultDuration;
            ...
            $http.get(args.templateUrl,{cache: $templateCache}).success(function(template){
                ...
                if (scope.$position === 'center' || scope.$position === 'top' ){
                    function positionAtCenter(){
                      scope.$centerMargin = '-' + (templateElement[0].offsetWidth /2) + 'px';
                    }
                    $timeout(positionAtCenter);
                    $timeout(positionAtCenter, 10);
                    $timeout(positionAtCenter, 40);
                }
                ...
                scope.$close = function(){
                    ...
                    layoutMessages();
                    if (scope.$onClose) {
                      scope.$onClose(args, scope);
                    }
                };

                scope.$clickAnyWhereToClose = args.clickAnyWhereToClose || false;
                scope.$notificationClicked = function() {
                  if (scope.$clickAnyWhereToClose) {
                    scope.$close();
                  }
                }
...
angular.module('cgNotify').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('angular-notify.html',
    ...
    "    $position === 'top' ? 'cg-notify-message-top' : '']\"\n" +
